...
# 强疑问标记：几乎一定是在「问」而不是在「要歌」，即便句中含“推荐/歌”等词
# 也应优先判为问答（例如“为什么推荐这三首？”“有什么理由吗”“这首歌是谁唱的”）。
...
def _is_question_intent(query: str, lowered: str) -> bool:
    ...
    优先级：
    1. 强疑问标记（为什么/理由/是谁/介绍…）最优先——即便句中含“推荐/歌”，
       也判为问答（例如“为什么会推荐这三首？”是在追问理由，不是要新歌）。
    2. 其次，明显的选歌意图（推荐/来点/想听…，或“有没有…歌”句式）判为选歌。
    3. 最后，普通疑问标记或以问号结尾判为问答。
    """
